...
module.exports = async (client, m, myPrefix, groupName = false, type = 1) => {
   ...
   if (!pc) {
      const group = groupName ? gray.bgYellow(' ' + await (await client.groupMetadata(m.chat)).subject + ' ') : '******'
      if (typeof m.text != 'object' && m.text.startsWith(myPrefix)) return console.log('\n' + yellow.bold('[ CMD ]'), Func.color(moment(time * 1000).format('DD/MM/YY HH:mm:ss'), 'green'), gray.bgGreen(' ' + m.mtype + ' '), green.bold('from'), '[' + m.sender.split`@` [0] + '] ' + gray.bgYellow(' ' + who + ' '), Func.color('in'), '[' + m.chat + '] ' + group, `\n${Func.mtype(m)}`)
      if (type == 1) console.log('\n' + blueBright.bold('[ MSG ]'), Func.color(moment(time * 1000).format('DD/MM/YY HH:mm:ss'), 'green'), gray.bgGreen(' ' + m.mtype + ' '), green.bold('from'), '[' + m.sender.split`@` [0] + '] ' + gray.bgYellow(' ' + who + ' '), Func.color('in'), '[' + m.chat + '] ' + group, `\n${Func.mtype(m)}`)
   } else if (pc) {
