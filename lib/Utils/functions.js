...
moment_timezone_1.default.tz.setDefault(process.env.TZ);
class Function {
    constructor() {
        this.delay = (time) => new Promise(res => setTimeout(res, time));
        this.createThumb = async (input, width = 200) => {
            try {
                const buffer = Buffer.isBuffer(input) ? input : this.isUrl(input) ? await this.fetchBuffer(input) : Buffer.alloc(0);
        ...
        this.downScale = async (input, scale = 2) => {
            try {
                const buffer = Buffer.isBuffer(input) ? input : this.isUrl(input) ? await this.fetchBuffer(input) : Buffer.alloc(0);
                const image = await read(buffer);
                ...
                const result = await image.getBufferAsync(MIME_JPEG);
                return result;
            }
            catch {
                return Buffer.alloc(0);
        ...
        this.fetchJson = async (url, options = {}) => {
            try {
                const result = await (await axios_1.default.get(url, {
                    ...options
                })).data;
                return result;
            }
        ...
        this.fetchBuffer = async (file, options = {}) => {
            try {
                if (Buffer.isBuffer(file)) {
                    ...
                    const buffer = await (await axios_1.default.get(file, {
        ...
        this.fetchAsBuffer = (source) => new Promise(async (resolve) => {
            try {
                if (this.isUrl(source)) {
                    const buffer = await (await (0, node_fetch_1.default)(source)).buffer();
                    resolve(buffer);
                }
                else {
        ...
        this.fetchAsJSON = (source) => new Promise(async (resolve) => {
            try {
                const result = await (await (0, node_fetch_1.default)(source)).json();
                resolve(result);
            }
            catch (e) {
                resolve(null);
        ...
        this.fetchAsText = (source) => new Promise(async (resolve) => {
            try {
                const result = await (await (0, node_fetch_1.default)(source)).text();
                resolve(result);
            }
            catch (e) {
                resolve(null);
        ...
        this.fetchAsBlob = (source) => new Promise(async (resolve) => {
            try {
                const result = await (await (0, node_fetch_1.default)(source)).blob();
                resolve(result);
            }
            catch (e) {
                resolve(null);
        ...
        this.parseCookie = async (file, options = {}) => {
            return new Promise(async (resolve, reject) => {
                try {
                    let cookie = await (await axios_1.default.get(file, {
                        responseType: "arraybuffer",
                        headers: options
                    })).headers['set-cookie'];
                    resolve(cookie);
        ...
        this.metaAudio = (source, tags = {}) => new Promise(async (resolve) => {
            try {
                ...
                if (/video/.test(mime)) {
                    ...
                    isFile = fs_1.default.readFileSync(file);
                }
                node_id3_1.default.write(tags, isFile, (e, buffer) => {
        ...
        this.example = (isPrefix, command, args) => {
        ...
        this.getSize = async (str) => {
            if (!isNaN(Number(str)))
                return this.formatSize(Number(str));
            let header = await (await axios_1.default.get(str.toString())).headers;
            return this.formatSize(Number(header?.['content-length'] || 0));
        };
        this.getFile = async (source, filename, options) => {
            try {
                if (Buffer.isBuffer(source)) {
                    let ext, mime;
                    try {
                        const bufferResult = await (0, file_type_1.fromBuffer)(source);
                    ...
                    const file = fs_1.default.writeFileSync(filepath, source);
                    const name = filename || path_1.default.basename(filepath);
                    ...
                    const extension = filename ? filename.split('.')[filename.split('.').length - 1] : ext;
                    const size = fs_1.default.statSync(source).size;
                    const name = filename || path_1.default.basename(source);
                    return new Promise(resolve => {
                        const data = {
                            status: true,
                            file: source,
                    ...
                    return new Promise(async (resolve) => {
                        try {
                            ...
                            res.on('response', (response) => {
                                ...
                                const extension = filename ? filename.split('.')[filename.split('.').length - 1] : mime_types_1.default.extension(response.headers['content-type']);
                                const file = fs_1.default.createWriteStream(`temp/${this.uuid() + '.' + extension}`);
                                const name = filename || path_1.default.basename(file.path?.toString());
                                const transformStream = new stream_1.default.Transform({
                                ...
                                file.on('finish', () => {
                                    const data = {
                                        ...
                                        file: file.path,
                                        filename: name,
                                        mime: mime_types_1.default.lookup(file.path),
                                        extension: extension,
        ...
        this.color = (text, color = 'green') => {
            return chalk_1.default?.[color]?.bold(text);
        };
        this.mtype = (data) => {
        ...
        this.reload = (file, delay = 1000) => {
            if (path_1.default.resolve(file) === __filename) {
                console.log(chalk_1.default.redBright.bold('[ SKIP ]'), chalk_1.default.blueBright((0, moment_timezone_1.default)(Date.now()).format('DD/MM/YY HH:mm:ss')), chalk_1.default.green.bold('File cannot reload itself: ' + path_1.default.basename(file)));
                return;
            }
            ...
            const watcher = chokidar_1.default.watch(file, { persistent: true, usePolling: false });
            const debouncedReload = this.debounce(() => {
                console.log(chalk_1.default.redBright.bold('[ UPDATE ]'), chalk_1.default.blueBright((0, moment_timezone_1.default)(Date.now()).format('DD/MM/YYYY HH:mm:ss')), chalk_1.default.green.bold('~ ' + path_1.default.basename(file)));
        ...
        this.updateFile = (file) => {
            ...
            const watcher = fs_1.default.watch(file, (eventType) => {
                if (eventType === 'change') {
                    console.log(chalk_1.default.redBright.bold('[ UPDATE ]'), chalk_1.default.blueBright((0, moment_timezone_1.default)(Date.now()).format('DD/MM/YYYY HH:mm:ss')), chalk_1.default.green.bold('~ ' + path_1.default.basename(file)));
                    delete require.cache[require.resolve(file)];
                    require(file);
        ...
        this.removeItem = (arr, value) => {
            let index = arr.indexOf(value);
            if (index > -1)
                arr.splice(index, 1);
            return arr;
        };
        this.hitstat = (cmd, who, options = {}) => {
            if (/bot|help|menu|stat|hitstat|hitdaily/.test(cmd))
                return;
        ...
        this.matcher = (string, array, options) => {
        ...
        this.timeFormat = (value) => {
            ...
            if (seconds < 10)
                seconds = '0' + seconds;
            if (hours == parseInt('00'))
                return minutes + ':' + seconds;
            return hours + ':' + minutes + ':' + seconds;
        };
        this.switcher = (status, isTrue, isFalse) => {
        ...
        this.greeting = () => {
            let time = parseInt(moment_timezone_1.default.tz(process.env.TZ || 'Asia/Jakarta').format('HH'));
        ...
        this.leveling = (xp, multiplier, def = 87200) => {
        ...
        this.removeEmojis = (string) => {
            var regex = /(?:[\u2700-\u27bf]|(?:\ud83c[\udde6-\uddff]){2}|[\ud800-\udbff][\udc00-\udfff]|[\u0023-\u0039]\ufe0f?\u20e3|\u3299|\u3297|\u303d|\u3030|\u24c2|\ud83c[\udd70-\udd71]|\ud83c[\udd7e-\udd7f]|\ud83c\udd8e|\ud83c[\udd91-\udd9a]|\ud83c[\udde6-\uddff]|\ud83c[\ude01-\ude02]|\ud83c\ude1a|\ud83c\ude2f|\ud83c[\ude32-\ude3a]|\ud83c[\ude50-\ude51]|\u203c|\u2049|[\u25aa-\u25ab]|\u25b6|\u25c0|[\u25fb-\u25fe]|\u00a9|\u00ae|\u2122|\u2139|\ud83c\udc04|[\u2600-\u26FF]|\u2b05|\u2b06|\u2b07|\u2b1b|\u2b1c|\u2b50|\u2b55|\u231a|\u231b|\u2328|\u23cf|[\u23e9-\u23f3]|[\u23f8-\u23fa]|\ud83c\udccf|\u2934|\u2935|[\u2190-\u21ff])/g;
            return string.replace(regex, '');
        };
        this.reSize = async (buffer, x, z) => {
            return new Promise(async (resolve, reject) => {
        ...
        this.logFile = (log, filename = 'bot') => {
            const log_file = fs_1.default.createWriteStream(path_1.default.join(process.cwd(), `${filename}.log`), {
                flags: 'a'
            });
            const time = (0, moment_timezone_1.default)(Date.now() * 1).format('DD/MM/YY HH:mm:ss');
        ...
        this.getDevice = (id) => id.length > 21 ? 'Android' : id.substring(0, 2) === '3A' ? 'IOS' : 'WhatsApp Web';
    }
}
