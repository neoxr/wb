        ...
        constructor(args = {}, options = {}) {
            ...
            this.init = async () => {
                try {
                    ...
                    if (typeof this?.session === 'string') {
                        var { state, saveCreds } = await useMultiFileAuthState(this.sf || this.session);
                    }
            ...
            this.socket = async ({ state, saveCreds, deleteCreds, autoDeleteOldData, restoreCreds }) => {
                ...
                this.sock.ev.on('connection.update', async (update) => {
                    ...
                    if (connection === 'connecting') {
                        ...
                        this.initEvent();
                        if (this.sf || this.session) {
                            session_1.default.backup(this.sock, this.sf || this.session);
                        }
                        this.initPlugin();
                        ...
                        if (reason === DisconnectReason.badSession) {
                            ...
                            if (this.sf || (this.session && typeof this.session === 'string')) {
                                this.clearSession({ deleteCreds });
                                const check = await session_1.default.isBackupExist(this.sock);
                                if (check) {
                                    await session_1.default.restore(this.sock, (this.sf || this.session));
                                    await delay(1500);
                                    this.init();
                                }
                            }
            ...
            this.clearSession = async ({ deleteCreds }) => {
                if (deleteCreds) {
                    ...
                    const PATH_BASE = path_1.default.join(process.cwd(), this.sf || this.session);
