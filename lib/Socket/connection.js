...
const baileys = fs_1.default.existsSync('./node_modules/@adiwajshing/baileys')
    ...
    : fs_1.default.existsSync('./node_modules/@neoxr/baileys')
        ...
        : fs_1.default.existsSync('./node_modules/@whiskeysockets/baileys')
            ...
            : require('baileys');
const { makeWASocket, Browsers, DisconnectReason, fetchLatestBaileysVersion, useMultiFileAuthState, makeCacheableSignalKeyStore, delay, proto, getAggregateVotesInPollMessage } = baileys;
...
const config = fs_1.default.existsSync('./config.json') ? JSON.parse(fs_1.default.readFileSync('./config.json', 'utf-8')) : {};
        ...
        constructor(args = {}, options = {}) {
            ...
            this.socket = async ({ state, saveCreds, deleteCreds, autoDeleteOldData, restoreCreds }) => {
                ...
                this.sock.ev.on('connection.update', async (update) => {
                    try {
                        const Session = new session_1.default(this.session);
                        Object.assign(this.store.state, update);
                        ...
                        if (connection === 'connecting') {
                            ...
                            this.initEvent();
                            Session.backup(this.sock, this.session);
                            this.initPlugin();
                            ...
                            if (reason === DisconnectReason.badSession) {
                                ...
                                if (typeof this.session === 'string') {
                                    ...
                                    const check = await Session.isBackupExist(this.sock);
                                    if (check) {
                                        await Session.restore(this.sock, this.session);
                                        await delay(1500);
                                        this.init();
                                    }
                                }
            ...
            this.busEvents = () => [
                {
                    ...
                    execute: async (chatUpdate) => {
                        ...
                        if (!m.message)
                            return;
                        if (['notify', 'append'].includes(chatUpdate?.type) && m?.key?.remoteJid) {
                            this.store.messages?.[m.key.remoteJid] ? this.store.messages[m.key.remoteJid].push(m) : this.store.messages[m.key.remoteJid] = [m];
                        }
                        this.initSetting();
                ...
                },
                {
                    ...
                    execute: async (chatUpdate) => {
                        for (const { key, update } of chatUpdate) {
                            if (update?.messageStubType === 1 && !isDeleted.has(key.id)) {
                                isDeleted.add(key.id);
                ...
                },
                {
                    ...
                    execute: ({ id, presences: update }) => {
                        this.store.presences[id] = this.store.presences[id] || {};
                        Object.assign(this.store.presences[id], update);
                    }
                },
                {
                    ...
                    execute: (update) => {
                        for (const { key, receipt } of update) {
                            const msg = this.store.loadMessage(key.remoteJid, key.id);
                            if (msg) {
                                this.store.updateMessageWithReceipt(msg, receipt);
                            }
                        }
                ...
                },
                {
                    ...
                    execute: (update) => {
                        for (const { key, reaction } of update) {
                            const msg = this.store.loadMessage(key.remoteJid, key.id);
                            if (msg) {
                                this.store.updateMessageWithReaction(msg, reaction);
                            }
                        }
                        this.emit('message.reaction', update);
                    }
                },
                {
                    event: 'contacts.upsert',
                    execute: (update) => {
                        this.store.contactsUpsert(update);
                    }
                },
                {
                    ...
                    execute: (updates) => {
                        for (const update of updates) {
                            if (update.id) {
                                let id = this.sock.decodeJid(update.id);
                                this.store.contacts = this.store.contacts || {};
                                if (this.store.contacts[id]) {
                                    Object.assign(this.store.contacts[id], update);
                                }
                                else {
                                    this.store.contacts[id] = {};
                                    Object.assign(this.store.contacts[id], { id, notify: update.notify });
                                    // console.info({ update }, 'got update for non-existent contact')
                                }
                            }
                            else { }
                        }
                ...
                },
                {
                    ...
                    execute: (update) => {
                        try {
                            const jid = update[0]?.id;
                            if (jid) {
                                this.store.chats[jid] = update[0];
                            }
                            // this.store.chats = this.store.chats || []
                            // this.store.chats = [...new Set([...this.store.chats, update[0]])]
                        }
                        catch (e) {
                            console.error(e);
                        }
            ...
            this.pairingConfig = this.options?.pairing ? this.options : config;
            this.setting = global.db?.setting && Object.values(global.db.setting).length > 0 ? global.db.setting : (this.options.setup || {
                multiprefix: true,
                prefix: ['.', '#', '!', '/'],
                onlyprefix: '+',
